<script>
	...
	onMount(async () => {
		...
		post = JSON.parse(JSON.stringify(data.postInfo));
...
<div
	class="flex items-start justify-center md:justify-between md:py-4  gap-4 flex-col  w-96 lg:w-128 md:mr-32 lg:mr-0 xl:max-w-9/12"
>
	<!-- post outline-->
				...
				{#each comments as comment}
					<div class="flex items-center">
						...
						<h4 class="flex-1 text-sm mx-2 ">
							<a href="profile/{comment.username}" class="font-semibold text-base"
								>{comment.username}</a
							>
