<script context="module">
	export const load = async ({ params, fetch }) => {
		const res = await fetch(`/api/liked-by/${params.postId}`, {
			credentials: 'include'
		});
		let people = null;
		if (res.status == 200) {
...
<svelte:head>
	<title>Post like by</title>
</svelte:head>

<div class="  md:w-fit w-full lg:w-128 my-2">
	<h1 class="text-white text-center text-lg py-4">Post like by</h1>
	{#if people}
		{#each people as friend}
			<section
